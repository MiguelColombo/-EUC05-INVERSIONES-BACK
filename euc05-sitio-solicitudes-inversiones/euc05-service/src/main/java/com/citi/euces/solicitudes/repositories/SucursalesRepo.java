...
public interface SucursalesRepo extends JpaRepository<Sucursales, Integer>{
	...
	@Query(value = "select * from PER_CAT_SUCURSALES s where s.SIRH_SUCURSAL_ID =:SIRH order by s.SIRH_SUCURSAL_ID ",nativeQuery = true )
	List<Sucursales> ObtenerSucursalesPorSuc(@Param("SIRH") BigInteger SIRH);
