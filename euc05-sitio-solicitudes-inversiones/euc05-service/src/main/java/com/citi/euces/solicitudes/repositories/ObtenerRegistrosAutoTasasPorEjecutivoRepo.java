...
public interface ObtenerRegistrosAutoTasasPorEjecutivoRepo extends JpaRepository<AutoTasasPorEjecutivo, Integer>{
	...
	@Query(value = " SELECT tas.ID_TASAUTO, tas.ESTATUS,tas.NUM_AUTORI_UEC,tas.FECHA_SOLIC, tas.FECHA_AUTORI, tas.FECHA_ESTATUS, "
            ...
            + " (CASE WHEN tas.TIPO_AUTORI IN ('DIVISIONALES','DISTRITALES','EXCEPGERENCIA') THEN tas.ID_CAMPANA "
            + " ELSE ofe.OFERTA_CAMPANIA_ID END) as ID_CAMPANA, ofe.OFERTA_SIGUIENTE_PASO AS OFERTA_SIGUIENTE_PASO, ofe.OFERTA_PDF_ESPECIAL_ID AS OFERTA_PDF_ESPECIAL_ID,"
            + " tas.NOMEJEC, tas.GATNOMINAOFER, tas.GATREALOFER,tas.RENDIMIENTOBRUTO ,cam.CAMPANIAS_FIN "
			+ " FROM UEC_TB_AUTOTASAS tas  inner join PER_CAT_SUCURSALES sucu on tas.SUC_SOLIC = sucu.SIRH_SUCURSAL_ID "
            + " left join PER_CAT_OFERTA ofe on ofe.OFERTA_ID =tas.TIPO_AUTORI "
            + " left join PER_CAT_CAMPANIAS cam on cam.CAMPANIAS_ID = ofe.OFERTA_CAMPANIA_ID "
		...
		//	+ " where 1" 
            + " where tas.NOMINA = :nomina and tas.NUM_CTE like %:num_cte% "
	...
	@Query(value =  " SELECT tas.ID_TASAUTO, tas.ESTATUS,tas.NUM_AUTORI_UEC,tas.FECHA_SOLIC, tas.FECHA_AUTORI, tas.FECHA_ESTATUS, "
		...
		//	+ " where 1" 
            ...
            + " and to_char(tas.FECHA_SOLIC, 'YYYY') = :year order by tas.FECHA_SOLIC desc"  
            ,nativeQuery = true )
	List<AutoTasasPorEjecutivo> ObtenerRegistrosAutoTasasPorEjecutivo2(@Param("nomina") String nomina,@Param("num_cte") String num_cte,@Param("year") String year);
	...
	@Query(value =  " SELECT tas.ID_TASAUTO, tas.ESTATUS,tas.NUM_AUTORI_UEC,tas.FECHA_SOLIC, tas.FECHA_AUTORI, tas.FECHA_ESTATUS, "
            ...
            + " left join UEC_TB_AUTORIZADORES_ELEGIDOS eleg on tas.ID_TASAUTO = eleg.id_TasaAuto " 
		//	+ " where 1" 
            + " where tas.NOMINA = :nomina and tas.NUM_CTE = :num_cte "
            + " and to_char(tas.FECHA_SOLIC, 'YYYY') = :year order by tas.FECHA_SOLIC desc"  
            ,nativeQuery = true )
	List<AutoTasasPorEjecutivo> ObtenerRegistrosAutoTasasPorEjecutivo3(@Param("nomina") String nomina,@Param("num_cte") String num_cte,@Param("year") String year);
}
